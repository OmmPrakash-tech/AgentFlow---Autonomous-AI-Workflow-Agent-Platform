...
class PlatformController {
 ...
 final Path root;
 PlatformController(Access access,Projects p,Definitions d,Accounts a,Audits audits,ResetTokens resets,ObjectMapper json,@Value("${agentflow.workspace-root}") String root) {
  this.access=access; projects=p; definitions=d; accounts=a; this.audits=audits; this.resets=resets; this.json=json; this.root=Path.of(root).toAbsolutePath().normalize();
 }
 ...
 @GetMapping("/{registry:agents|tools|workflows}") Object definitions(Authentication auth,@PathVariable String registry) {
  var a=access.user(auth); access.require(a,"READ"); return definitions.findByKindOrderByName(kind(registry)).stream().map(this::definition).toList();
 }
