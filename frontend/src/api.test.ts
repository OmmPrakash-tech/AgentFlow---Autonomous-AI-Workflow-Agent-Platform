...
describe("authenticated API client", () => {
  it("attaches bearer credentials and surfaces server rejection", async () => {
    ...
    setToken("test-token");
    await expect(api("/agents", {})).rejects.toThrow("Permission denied");
  ...
  it("parses authenticated SSE state without URL tokens", async () => {
    const fetch = vi.fn().mockResolvedValue({
      ok: true,
      text: async () => 'event: state\ndata: {"status":"COMPLETED"}\n\n',
    });
    vi.stubGlobal("fetch", fetch);
    const { stateStream } = await import("./api");
    expect(await stateStream("run-id", new AbortController().signal)).toEqual({
      status: "COMPLETED",
    });
