...
Follow-up discovery: PostgreSQL 18.3 is installed at `C:\Program Files\PostgreSQL\18\bin`, with an existing service. A separate password-protected test cluster was initialized under ignored `work/postgres-test` on loopback port 55432. The existing service/databases were not used or modified. The Python 3.13 directory was present but did not contain a runnable Python executable; Python 3.14 was used.
